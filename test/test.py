...
def module_setup(request, device, data_dir, platform_data_dir, app_dir, artifact_dir):
    def module_teardown():
        ...
        device.run_ssh('cp -r /var/snap/jitsimeet/current/config {0}/config.current'.format(TMP_DIR), throw=False)
        device.run_ssh('cp -r /snap/jitsimeet/current/config {0}/config.app'.format(TMP_DIR), throw=False)
